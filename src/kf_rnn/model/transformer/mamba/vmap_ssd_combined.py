...
from mamba_ssm.ops.triton.ssd_bmm import _bmm_chunk_bwd
...
from .vmap_chunk_fn import (
    vmap_bmm_chunk_fwd,
    vmap_chunk_scan_bwd_dstates,
    vmap_chunk_scan_chunk_state_bwd_dx,
    vmap_chunk_scan_bwd_dC,
    vmap_chunk_scan_bwd_ddAcs_stable,
)
...
def _mamba_chunk_scan_combined_fwd(
    ...
    if B.stride(-1) != 1:
        B = B.contiguous()
    if C_list.stride(-1) != 1:
        C_list = C_list.contiguous()
    ...
    states, final_states = _state_passing_fwd(rearrange(states, "... p n -> ... (p n)"), dA_cumsum[:, :, :, -1],
                                              ...
                                              seq_idx=seq_idx, chunk_size=chunk_size, out_dtype=C_list.dtype)
    states, final_states = [rearrange(t, "... (p n) -> ... p n", n=dstate) for t in [states, final_states]]
    # states_tmp0 = rearrange(_state_passing_fwd(rearrange(states_tmp0, "... p n -> ... (p n)"), dA_cumsum_tmp0[:, :, :, -1], chunk_size=chunk_size), "... (p n) -> ... p n", n=dstate)
    # states_tmp1 = rearrange(_state_passing_fwd(rearrange(states_tmp1, "... p n -> ... (p n)"), dA_cumsum_tmp1[:, :, :, -1], chunk_size=chunk_size), "... (p n) -> ... p n", n=dstate)
    
    CB_list = vmap_bmm_chunk_fwd(C_list, B, chunk_size, seq_idx=seq_idx, output_dtype=torch.float32)
    _out: list[tuple[torch.Tensor, torch.Tensor]] = []
    for _C, _CB in zip(C_list, CB_list):
        _out.append(_chunk_scan_fwd(_CB, x, dt, dA_cumsum, _C, states, D=D, z=z, seq_idx=seq_idx))
    out_list, out_x_list = zip(*_out)
    out_list = torch.stack(out_list, dim=0)
    out_x_list = None if out_x_list[0] is None else torch.stack(out_x_list, dim=0)
...
def _mamba_chunk_scan_combined_bwd(
    ...
    dstates = vmap_chunk_scan_bwd_dstates(C_list, dA_cumsum, dout, seq_idx=seq_idx, dtype=states.dtype)
    ...
    CB_list = vmap_bmm_chunk_fwd(C_list, B, chunk_size, seq_idx=seq_idx, output_dtype=torch.float32)
    dx, ddt, dD_from_x = vmap_chunk_scan_chunk_state_bwd_dx(x, dt, dA_cumsum, B, CB_list, dout, dstates, D=D, seq_idx=seq_idx, dx=dx)
    ...
    dC_list, ddA_cumsum_prev = vmap_chunk_scan_bwd_dC(states.to(x.dtype), dA_cumsum, dout, seq_idx=seq_idx, C_list=C_list, ngroups=ngroups)
    ...
    dCB = dCB.to(CB_list.dtype)
    
    dB_given = torch.zeros_like(B)
    for _C in C_list:
        dB_given += _bmm_chunk_bwd(_C, dCB, residual=dB)
    
    dC_given_list = torch.stack([
        _bmm_chunk_bwd(B, rearrange(dCB, "... l s -> ... s l"), residual=_dC)
        for _dC in dC_list
    ], dim=0)
    ...
    ddA = vmap_chunk_scan_bwd_ddAcs_stable(x, dt, dA_cumsum, dout, CB_list)
    ...
    return_vals = (dx, ddt_given, dA, dB_given, dC_given_list, dD, dz, ddt_bias, dinitial_states)
...
class MambaChunkScanCombinedFn(torch.autograd.Function):
    ...
    def forward(
        ...
        out, out_x, dt_out, dA_cumsum, states, final_states, *rest = _mamba_chunk_scan_combined_fwd(x, dt, A, B, C_list, chunk_size, D=D, z=z, dt_bias=dt_bias, initial_states=initial_states, seq_idx=seq_idx, cu_seqlens=cu_seqlens, dt_softplus=dt_softplus, dt_limit=dt_limit)
        ctx.save_for_backward(out if z is None else out_x, x, dt, dA_cumsum, A, B, C_list, D, z, dt_bias, initial_states, seq_idx)
    ...
    def backward(ctx, dout, *args):
        out, x, dt, dA_cumsum, A, B, C_list, D, z, dt_bias, initial_states, seq_idx = ctx.saved_tensors
