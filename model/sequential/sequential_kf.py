...
class SequentialKF(KF):
    ...
    def forward(self, trace: Dict[str, torch.Tensor], mode: str = None) -> Dict[str, torch.Tensor]:
        ...
        if mode == "sequential":
            ...
            for state_biases, observation_biases in zip(state_biases_list, observation_biases_list):
                ...
                observation_estimations.append(SequentialKF._evaluate_form(state, (observation_weights[:observation_biases.shape[1]], observation_biases)))
    ...
    def _forward_generic(self,
        ...
        buffered_state_weights = torch.cat([state_weights, torch.zeros((1, self.S_D, self.S_D))])                                       # [(subT + 2) x S_D x S_D]
        lower_triangular_indices = (torch.arange(subT)[:, None] - torch.arange(subT)).clamp_min(-1)                                     # [subT x sqrtT]
        ...
        if mode == "form":
            ...
            reshaped_state_biases = torch.cat([
                torch.zeros((B * lsqrtT, 1, self.S_D)),
                ((reshaped_padded_inputs @ (E @ self.B).mT + reshaped_padded_observations @ self.K.mT).view(B * lsqrtT, -1) @ lower_triangular_matrix.T).view(B * lsqrtT, hsqrtT, self.S_D)
