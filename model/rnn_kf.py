...
class RnnKF(KF):
    ...
    def forward(self, trace: Dict[str, torch.Tensor], mode: str = None) -> Dict[str, torch.Tensor]:
        ...
        if mode == 'sequential':
            ...
            for state_biases, observation_biases in zip(state_biases_list, observation_biases_list):
                state_estimations.append(RnnKF.evaluate_form(state, (state_weights[:state_biases.shape[1]], state_biases)))
                observation_estimations.append(RnnKF.evaluate_form(state, (observation_weights[:observation_biases.shape[1]], observation_biases)))
    ...
    def forward_generic(self,
        ...
        lower_triangular_indices = torch.arange(subT, device=device)[:, None] - torch.arange(subT, device=device)                   # [subT x sqrtT]
